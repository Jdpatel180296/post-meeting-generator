// server/db/migrations/20251115_02_add_features.js
exports.up = async function (knex) {
  // Social media accounts (LinkedIn, Facebook credentials)
  const hasSocial = await knex.schema.hasTable("social_media_accounts");
  if (!hasSocial) {
  ...
  const hasAutomations = await knex.schema.hasTable("automations");
  if (!hasAutomations) {
  ...
  const hasSettings = await knex.schema.hasTable("user_settings");
  if (!hasSettings) {
